...
class Qwen25OmniClient:
    @property
    def model_name(self) -> str:
        return "qwen2_5_omni"

    def predict(self, request: InferenceRequest) -> InferenceResult:
        model_config = CONFIG.model("qwen2_5_omni")
        server_url = request.metadata.get("server_url") if request.metadata else None
        server_url = server_url or model_config.get("server_url") or os.getenv("QWEN2_5_OMNI_SERVER_URL")
        if not server_url:
            raise ValueError("缺少 Qwen2.5-Omni server_url，请在 config/config.yaml 或环境变量中配置。")
