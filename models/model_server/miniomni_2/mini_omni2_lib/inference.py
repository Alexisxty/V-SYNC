...
def load_audio(path, max_duration_sec=30):
    """Load audio file and convert to mel spectrogram.

    Args:
        path: Path to audio file
        max_duration_sec: Maximum duration in seconds. Default 30 (Whisper's limit).
                         Audio longer than this will be truncated.

    Returns:
        mel: Mel spectrogram
        leng: Number of frames (capped at max_duration_sec * 50)
    """
    audio = whisper.load_audio(path)
    duration_ms = (len(audio) / 16000) * 1000

    # Whisper encoder has fixed positional embeddings for 30s
    # Truncate to max_duration_sec to avoid shape mismatch
    audio = whisper.pad_or_trim(audio, length=int(max_duration_sec * 16000))

    mel = whisper.log_mel_spectrogram(audio)

    # Return actual frame count (may be less than max if audio is shorter)
    actual_duration_ms = min(duration_ms, max_duration_sec * 1000)
    return mel, int(actual_duration_ms / 20) + 1
